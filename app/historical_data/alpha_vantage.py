...
class AlphaVantageHistoricalProvider(HistoricalDataProvider):
    ...
    def _payload_warnings(self,payload, expected_key=None):
        ...
        if expected_key and isinstance(payload,dict) and expected_key not in payload and not warnings: warnings.append(f"Alpha Vantage payload missing expected key: {expected_key}")
    ...
    def parse_daily_adjusted(self, instrument, payload, start_date, end_date):
        warnings=self._payload_warnings(payload,"Time Series (Daily)"); series=payload.get("Time Series (Daily)", {}) if isinstance(payload,dict) else {}
        bars=[]
        for d,b in sorted(series.items()):
            ...
            except Exception as exc: warnings.append(f"Malformed Alpha Vantage daily row skipped for {instrument}: {exc}")
        if not bars and not any("expected key" in w or "empty" in w or "Error" in w for w in warnings): warnings.append(f"No Alpha Vantage daily adjusted data for {instrument}")
    ...
    def parse_fx_daily(self, fc, tc, payload, start_date, end_date):
        warnings=self._payload_warnings(payload,"Time Series FX (Daily)"); series=payload.get("Time Series FX (Daily)", {}) if isinstance(payload,dict) else {}
        rates=[]
        for d,b in sorted(series.items()):
            try:
                dt=date.fromisoformat(d)
            ...
            except Exception as exc: warnings.append(f"Malformed Alpha Vantage FX row skipped for {fc}/{tc}: {exc}")
        if not rates and not any("expected key" in w or "empty" in w for w in warnings): warnings.append(f"No Alpha Vantage FX_DAILY data for {fc}/{tc}")
