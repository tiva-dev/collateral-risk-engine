...
class AlpacaTradingHistoricalProvider(HistoricalDataProvider):
    ...
    def __init__(self, cache: HistoricalDataCache | None = None):
        ...
        validate_provider_url(self.config.alpaca_base_url, {"data.alpaca.markets"})
        self.cache = cache or HistoricalDataCache()
        self.cache_paths: list[str] = []
        self.raw_response_paths: list[str] = []
        self.provider_coverage_summary: dict[str, Any] = {}
        self.total_api_call_count = 0

    def auth_headers(self) -> dict[str, str]:
        h = {}
        if self.config.alpaca_api_key:
            h["APCA-API-KEY-ID"] = self.config.alpaca_api_key
        if self.config.alpaca_secret_key:
            h["APCA-API-SECRET-KEY"] = self.config.alpaca_secret_key
        return h
    ...
    def _request_json(self, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise ProviderError(
                f"Alpaca request failed: {exc}", provider=self.provider_name
            ) from exc
        if (
            isinstance(payload, dict)
            and (payload.get("code") or payload.get("message"))
            and "bars" not in payload
        ):
            msg = str(payload.get("message") or payload)
            self.warnings.append(msg)
            raise ProviderError(
                f"Alpaca API error: {msg}",
    ...
    def _identity(self, symbol: str) -> InstrumentIdentity:
        asset_type = (
            AssetType.ETF
            if symbol.upper() in {"SPY", "QQQ", "IWM", "EFA", "VTI"}
            else AssetType.LISTED_EQUITY
        )
        return InstrumentIdentity(
            f"US:{symbol}:USD", symbol, "US", "USD", asset_type, provider_symbol=symbol
        )
    ...
    def _extract_symbol_bars(
        ...
        if isinstance(bars_payload, dict):
            return list(
                bars_payload.get(instrument)
                or bars_payload.get(instrument.upper())
                or []
            )
    ...
    def parse_bars(
        ...
        if not raw_bars:
            warnings.append(f"No Alpaca bars returned for {instrument}")
        ...
        return HistoricalSeries(
            ...
            data_quality_summary={
                "bar_count": len(bars),
                "page_count": self.provider_coverage_summary.get("page_count", 0),
    ...
    def _merge_payload(self, pages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for p in pages:
            ...
            if isinstance(bp, dict):
                for sym, rows in bp.items():
                    merged["bars"].setdefault(sym, []).extend(rows or [])
    ...
    def fetch_equity_history(
        ...
        if not force_refresh and (cached := self.cache.read("normalized", **key)):
            ...
            if raw_path.exists():
                self.raw_response_paths.append(str(raw_path))
            data = cached["data"]
            self.provider_coverage_summary.update(
                {
                    "requested": 1,
                    "available": 1,
                    "missing": 0,
        ...
        try:
            while True:
                q = {
                    "symbols": instrument,
                    "timeframe": "1Day" if interval in {"1d", "daily"} else interval,
            ...
            self.warnings.append(
                f"Alpaca pagination failed after {page_count} pages: {exc}"
            )
            if not pages:
                raise
        merged = self._merge_payload(pages)
        if "__single__" in merged["bars"]:
            merged["bars"] = {instrument: merged["bars"].pop("__single__")}
        merged["metadata"] = {
        ...
        self.provider_coverage_summary.update(
            {
                "requested": 1,
                "available": 1 if self._extract_symbol_bars(instrument, merged) else 0,
                "missing": 0 if self._extract_symbol_bars(instrument, merged) else 1,
                "cached": 0,
                "fetched": 1,
                "page_count": page_count,
        ...
        if not series.bars:
            raise ProviderError(
                f"Alpaca returned no covered bars for {instrument}",
                provider=self.provider_name,
                code="empty_coverage",
            )
...
def _series_from_cache(data):
    identity = (
        InstrumentIdentity(**data["instrument_identity"])
        if data.get("instrument_identity")
        else None
    )
    bars = []
    for row in data.get("bars", []):
        ri = (
            InstrumentIdentity(**row["instrument_identity"])
            if row.get("instrument_identity")
            else identity
        )
        bars.append(
            HistoricalBar(
