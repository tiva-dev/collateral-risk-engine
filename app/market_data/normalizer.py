...
class NormalizedMarketData:
    ...
    def _converted_order_book(self) -> OrderBook | None:
        if self.order_book is None:
            return None
        return OrderBook(
            bids=[
                OrderBookLevel(
                    price=self._convert_value(level.price),
...
def normalize_quote(
    ...
    target_currency = loan_currency.upper()
    fx_required = local_currency != target_currency
    fx_rate_value = (
        None
        if fx_rate is None or (fx_rate.rate == 1.0 and not fx_required)
        else fx_rate.rate
    )
    converted_price = quote.local_price
    if fx_required and fx_rate_value is None:
        converted_price = 0.0
    elif fx_rate_value is not None:
        converted_price = quote.local_price * fx_rate_value
    average_dollar_volume = quote.average_dollar_volume
    if average_dollar_volume is not None and fx_required and fx_rate_value is None:
        average_dollar_volume = 0.0
    elif average_dollar_volume is not None and fx_rate_value is not None:
        average_dollar_volume *= fx_rate_value
    return NormalizedMarketData(
        instrument=quote.instrument,
        local_price=quote.local_price,
        local_currency=local_currency,
        loan_currency=target_currency,
