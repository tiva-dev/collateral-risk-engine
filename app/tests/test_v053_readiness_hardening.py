...
class V053ReadinessHardeningTests(unittest.TestCase):
    ...
    def test_time_indexed_fx_inverse_and_stale_flags(self):
        ...
        scenario = OfficialPortfolioScenario(
            "fx_curve", [Holding("MTNN", AssetType.LISTED_EQUITY, 1, "NGN")], "USD"
        )
        result = HistoricalReplayEngine(seed=1).replay(
            scenario, {"MTNN": bars}, fx_rates=fx
        )
        self.assertAlmostEqual(result["records"][0]["collateral_value"], 0.1)
        self.assertFalse(result["records"][0]["fx_missing"])

        old_fx = {
            ("NGN", "USD"): [HistoricalFXRate("NGN", "USD", 0.001, date(2023, 12, 1))]
        }
        stale = HistoricalReplayEngine(seed=1).replay(
    ...
    def test_normalized_cache_loader_and_provider_native_warning_tolerance(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            def envelope(data):
                return {"checksum": content_hash(data), "data": data}
            ...
            fx_data = {
                ...
                "warnings": [],
                "data_quality_summary": {},
                "rates": [{"timestamp": "2024-01-01", "rate": 1.1}],
            }
            native_data = {"Time Series (Daily)": {"2024-01-01": {"4. close": "10"}}}
            bar_path = Path(td) / "bar.json"
            bar_path.write_text(json.dumps(envelope(bar_data)))
            fx_path = Path(td) / "fx.json"
            fx_path.write_text(json.dumps(envelope(fx_data)))
            native_path = Path(td) / "native.json"
            native_path.write_text(json.dumps(envelope(native_data)))
            bars, fx = _load_replay_inputs(
                {"cache_paths": [str(bar_path), str(fx_path), str(native_path)]}
            )
    ...
    def test_baselines_metrics_policy_interest_and_synthetic_thin(self):
        bars = {
            "AAPL": [
                HistoricalBar(
                    "AAPL", date(2024, 1, 1), 100, 100, 100, 100, volume=1000
                ),
                HistoricalBar("AAPL", date(2024, 2, 1), 80, 80, 80, 80, volume=1000),
            ]
        }
        conservative = OfficialPortfolioScenario(
            "p",
            [Holding("AAPL", AssetType.LISTED_EQUITY, 10)],
            "USD",
            base_ltv_policy=0.5,
            risk_appetite=RiskAppetite.CONSERVATIVE,
            loan_terms=InterestPolicy(0.36),
        )
        aggressive = OfficialPortfolioScenario(
            "p",
            [Holding("AAPL", AssetType.LISTED_EQUITY, 10)],
            "USD",
            base_ltv_policy=0.8,
            risk_appetite=RiskAppetite.AGGRESSIVE,
            loan_terms=InterestPolicy(0.0),
        )
        ...
        self.assertAlmostEqual(
            r1["baseline_results"]["flat_ltv"][0]["credit_limit"], 500.0
        )
        configured_flat = HistoricalReplayEngine(seed=1).replay(
            conservative, bars, flat_ltv=0.65
        )
        self.assertAlmostEqual(
            configured_flat["baseline_results"]["flat_ltv"][0]["credit_limit"], 650.0
        )
        self.assertNotEqual(
            r1["records"][0]["credit_limit"], r2["records"][0]["credit_limit"]
        )
        self.assertGreater(
            r1["records"][-1]["loan_balance"], r1["records"][-1]["principal"]
        )
        metrics = compute_simulation_metrics(r1)
        self.assertIn("dynamic_engine_versus_static_ltv_outcome_table", metrics)
        self.assertIn("credit_limit_breach_reduction_versus_flat_ltv", metrics)
        self.assertEqual(
            _synthetic_thin_bars(date(2024, 1, 1), date(2024, 1, 3), 7),
            _synthetic_thin_bars(date(2024, 1, 1), date(2024, 1, 3), 7),
        )
