...
class V061RealDataReadinessTests(unittest.TestCase):
    def test_alpha_vantage_fx_write_is_canonical_and_replayable(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(provider, "_request_json", return_value=response):
                provider.fetch_fx_history(
                    "EUR", "USD", date(2024, 1, 1), date(2024, 1, 3), force_refresh=True
                )
    ...
    def test_provider_native_cache_is_recorded_and_skipped(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            path = Path(td) / "native.json"
            path.write_text(json.dumps({"checksum": content_hash(data), "data": data}))
            ...
            self.assertFalse(bars)
            self.assertFalse(fx)
            self.assertTrue(manifest["qa_cache_warnings"])

    def test_calibration_does_not_substitute_loan_balance(self):
        d = generate_calibration_diagnostics(
            [
                {
                    "scenario": "s",
                    "average_loan_balance": 999,
                    "average_approved_credit": 100,
                    "average_lifecycle_safe_credit_limit": 80,
                }
            ]
        )["scenarios"]["s"]
        self.assertEqual(d["average_approved_credit_by_scenario"]["value"], 100)
        missing = generate_calibration_diagnostics(
            [{"scenario": "s", "average_loan_balance": 999}]
        )["scenarios"]["s"]
        self.assertEqual(
            missing["average_approved_credit_by_scenario"]["status"], "unavailable"
        )
    ...
    def test_workflow_dry_run_validation_guardrail(self):
        ...
        self.assertIn(
            "inputs.dry_run == 'true' && inputs.run_validation_after_build == 'true'",
            text,
        )
        self.assertIn(
            "Cannot run official validation after a dry-run dataset build because no provider cache data is generated.",
            text,
        )
