...
class MonitoringScheduler(Protocol):
    def next_check_after(self, margin_state: MarginState | None, now: datetime | None = None) -> datetime: ...
...
class SimpleMonitoringScheduler:
    ...
    def next_check_after(self, margin_state: MarginState | None, now: datetime | None = None) -> datetime:
        ...
        return base + self.intervals.get(margin_state or MarginState.SAFE, self.intervals[MarginState.SAFE])
